...
function matrisArreglo(matris) {
	...
	for (var i = 0; i < tmp.length; i++) {
		tmp[i] = Number(tmp[i]);
		tmp[i] = (tmp[i] > 0) ? Math.floor(tmp[i]) : Math.ceil(tmp[i]);
	}
	return tmp;
}
...
function Rubik(areaInicial, numeroInicial, rotacionInicial, funcionGiroInicial, funcionCompletoInicial) {
	...
	var limpiaSeleccion = function() {
		if (seleccion) {
			if (desarrollo)
				$(".seleccion1, .seleccion2, .rotar1X, .rotar1Y, .rotar1Z, .rotar2X, .rotar2Y, .rotar2Z, .direccion1, .direccion2").each(function() {
	...
	var movimientoAleatorio = function() {
		var elemento = $(".cara:eq(" + Math.floor(Math.random() * nCaras) + ")");
		seleccionar.call(elemento[0], {clientX: 0, clientY: 0});
		
		var direccion = "";
		if (Math.floor(Math.random() * 2)) {
			if ($(".direccion1").hasClass("rotar1X"))
				direccion = "X";
			else if ($(".direccion1").hasClass("rotar1Y"))
				direccion = "Y";
			else if ($(".direccion1").hasClass("rotar1Z"))
				direccion = "Z";
		} else {
			if ($(".direccion2").hasClass("rotar1X"))
				direccion = "X";
			else if ($(".direccion2").hasClass("rotar1Y"))
				direccion = "Y";
			else if ($(".direccion2").hasClass("rotar1Z"))
				direccion = "Z";
		}
	...
	var eventoTouchMove = function(evento) {
		if (evento.touches.length == 1)
			eventoMover.call(this, evento.touches[0]);
		else if (evento.touches.length == 2) {
			evento.touches[1].deltaY = 0;
			var distanciaSTmp = distancia({x1: evento.touches[0].clientX, y1: evento.touches[0].clientY, x2: evento.touches[1].clientX, y2: evento.touches[1].clientY});
			if (distanciaSTmp < distanciaS)
				evento.touches[1].deltaY = -1;
			else if (distanciaSTmp > distanciaS)
				evento.touches[1].deltaY = 1;
			distanciaS = distanciaSTmp;
			eventoEscalar.call(this, evento.touches[1]);
		}
	}
	
	var creaEventos = function() {
		if ('ontouchstart' in window) {
			$(rotacion)[0].addEventListener('touchstart', function(evento) {
				evento.preventDefault();
				if (evento.touches.length == 1)
					eventoTocarRotacion.call(this, evento.touches[0]);
				else if (evento.touches.length == 2) {
					mover = false;
					moverCara = false;
					distanciaS = distancia({x1: evento.touches[0].clientX, y1: evento.touches[0].clientY, x2: evento.touches[1].clientX, y2: evento.touches[1].clientY});
				}
			});
			$(".cara").each(function() {
				this.addEventListener('touchstart', function(evento) {
					evento.preventDefault();
					if (evento.touches.length == 1)
						eventoTocarCara.call(this, evento.touches[0]);
					else if (evento.touches.length == 2) {
						mover = false;
						moverCara = false;
						distanciaS = distancia({x1: evento.touches[0].clientX, y1: evento.touches[0].clientY, x2: evento.touches[1].clientX, y2: evento.touches[1].clientY});
			...
			$(rotacion)[0].addEventListener('touchmove', function(evento) {
				evento.preventDefault();
				eventoTouchMove.call(this, evento);
			});
			$(".cara").each(function() {
				this.addEventListener('touchmove', function(evento) {
					evento.preventDefault();
					eventoTouchMove.call(this, evento);
				});
			});
			$(rotacion)[0].addEventListener('touchend', function(evento) {
				evento.preventDefault();
				if (evento.touches.length == 0) {
					eventoSoltarCara.call(this, evento.changedTouches[0]);
					eventoSoltarRotacion.call(this, evento.changedTouches[0]);
				} else if (evento.touches.length == 1)
					eventoTocarRotacion.call(this, evento.touches[0]);
			});
			$(".cara").each(function() {
				this.addEventListener('touchend', function(evento) {
					evento.preventDefault();
					if (evento.touches.length == 0) {
						eventoSoltarRotacion.call(this, evento.changedTouches[0]);
						eventoSoltarCara.call(this, evento.changedTouches[0]);
			...
			$(".cara").mousewheel(function(evento) {
				evento.preventDefault();
				eventoEscalar.call(this, evento);
			...
			$(".cara").mousedown(function(evento) {
				evento.preventDefault();
			...
			$(".cara").mousemove(function(evento) {
				evento.preventDefault();
				eventoMover.call(this, evento);
			});
	...
	var crear = function() {
		...
		borde = (borde > 0) ? Math.floor(borde) : Math.ceil(borde);
		borde2 = (borde2 > 0) ? Math.floor(borde2) : Math.ceil(borde2);
	...
	this.eliminaEventos = function() {
		if ('ontouchstart' in window) {
			//alert("remover eventos unbind");
		} else {
			$(rotacion).unbind('mousewheel');
			$(".cara").unbind('mousewheel');
			$(rotacion).unbind('mousedown');
			$(".cara").unbind('mousedown');
			$(rotacion).unbind('mousemove');
			$(".cara").unbind('mousemove');
		...
		$(contenedor).animate({ valorTS: 100 }, {
			...
			step: function(now, fx) {
				$(".cara").each(function() {
					...
					if (matris[14] == borde)
						$(this).aplicaEstilo('transform', $(this).attr('valorInicialTS') + " translateX(" + rangoValor(0, (matris[12] * valor), now, fx['end']) + "px) translateY(" + rangoValor(0, (matris[13] * valor), now, fx['end']) + "px) translateZ(" + rangoValor(0, valorBorde, now, fx['end']) + "px) scale3d(" + rangoValor(escalar, valorEscalar, now, fx['end']) + ", " + rangoValor(escalar, valorEscalar, now, fx['end']) + ", " + rangoValor(escalar, valorEscalar, now, fx['end']) + ")");
					else if (matris[14] == -borde)
						$(this).aplicaEstilo('transform', $(this).attr('valorInicialTS') + " translateX(" + rangoValor(0, (matris[12] * valor), now, fx['end']) + "px) translateY(" + rangoValor(0, (-matris[13] * valor), now, fx['end']) + "px) translateZ(" + rangoValor(0, valorBorde, now, fx['end']) + "px) scale3d(" + rangoValor(escalar, valorEscalar, now, fx['end']) + ", " + rangoValor(escalar, valorEscalar, now, fx['end']) + ", " + rangoValor(escalar, valorEscalar, now, fx['end']) + ")");
					else if (matris[13] == borde)
		...
		$(contenedor).animate({ valorTS: 100 }, {
			step: function(now, fx) {
				$(".cara").each(function() {
					var matris = matrisArreglo($(this).attr('valorInicialTS'));
					if (matris[14] == borde)
						$(this).aplicaEstilo('transform', $(this).attr('valorInicialTS') + " translateX(" + rangoValor((matris[12] * valor), 0, now, fx['end']) + "px) translateY(" + rangoValor((matris[13] * valor), 0, now, fx['end']) + "px) translateZ(" + rangoValor(valorBorde, 0, now, fx['end']) + "px) scale3d(" + rangoValor(valorEscalar, escalar, now, fx['end']) + ", " + rangoValor(valorEscalar, escalar, now, fx['end']) + ", " + rangoValor(valorEscalar, escalar, now, fx['end']) + ")");
					else if (matris[14] == -borde)
						$(this).aplicaEstilo('transform', $(this).attr('valorInicialTS') + " translateX(" + rangoValor((matris[12] * valor), 0, now, fx['end']) + "px) translateY(" + rangoValor((-matris[13] * valor), 0, now, fx['end']) + "px) translateZ(" + rangoValor(valorBorde, 0, now, fx['end']) + "px) scale3d(" + rangoValor(valorEscalar, escalar, now, fx['end']) + ", " + rangoValor(valorEscalar, escalar, now, fx['end']) + ", " + rangoValor(valorEscalar, escalar, now, fx['end']) + ")");
					else if (matris[13] == borde)
